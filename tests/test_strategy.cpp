...
TEST_F(StrategyTest, ReturnsUnknownWhenNotEnoughData) {
    ...
    Tick t{0, 100.0, 1.0, Side::Unknown};
...
TEST_F(StrategyTest, TriggersSellOnSpikeUp) {
    ...
    Tick stableTick{0, 100.0, 1.0, Side::Unknown};
    for(int i=0; i<5; ++i) {
        strategy.onTick(stableTick);
    }

    Tick spikeTick{0, 110.0, 1.0, Side::Buy};
...
TEST_F(StrategyTest, TriggersBuyOnDrop) {
    ...
    Tick dropTick{0, 90.0, 1.0, Side::Sell};
...
TEST_F(StrategyTest, StaysQuietWithinBands) {
    ...
    for(int i=0; i<20; ++i) {
        Tick t{0, 100.0 + (i%2 == 0 ? 1.0 : -1.0), 1.0, Side::Unknown}; 
        strategy.onTick(t);
    }
}